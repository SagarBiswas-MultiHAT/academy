...
| Product Tier             | Description                                                                                                            | Price (USD)                        |
| :----------------------- | :--------------------------------------------------------------------------------------------------------------------- | :--------------------------------- |
| **Free Web Chapters**    | First three chapters of each notebook, hosted on the website as HTML/Markdown, with embedded author branding and CTAs. | Free                               |
| **Premium E‑Book (PDF)** | Full, printable, searchable PDF with buyer‑specific dynamic watermark and bonus resources.                             | \$5–\$15                           |
| **Certification Kit**    | PDF + interactive quiz + personalized, verifiable certificate.                                                         | \$19–\$29 (bundled) or \$10 add‑on |
| **Future Membership**    | Monthly subscription granting access to all notebooks, exclusive content, and a private community.                     | \$8/month                          |

### 2.2 Revenue Streams

- Direct e‑book sales via aamarPay.
- Certification kit upsells.
- Corporate team licenses (e.g., cybersecurity firms training interns).
- Affiliate referrals (future).
- Subscription recurring revenue (future).
...
| Core Modules   | `PaymentsModule` (aamarPay integration), `BooksModule` (product CRUD), `CertificatesModule` (quiz scoring, PDF generation, verification), `UsersModule` (profile, purchase history) |
...
| :--------------- | :----------------------------------------------------------------------------------- | :------------------------------------- |
| `users`          | Registered users: email, hashed password, profile, role                             | Has many `orders`, `certificates`      |
| `books`          | Product catalog: title, slug, description, price, chapter metadata                  | Has many `orders`, `quiz_questions`    |
| `orders`         | Purchase records: amount, payment status, transaction ID, gateway response          | Belongs to `user`, `book`              |
...
Orders:        POST /api/v1/orders (initiate purchase → returns aamarPay URL)
...
- Implement referral tracking with UTM‑based attribution.
