# Production Deployment Guide (Step 9)

This guide follows the deployment strategy from **Step 9** in `instructions.md`, adapted to run **both** the Next.js frontend and NestJS backend behind **one domain**:

- **Public site + app**: `https://academy.yourdomain.com/`
- **API** (proxied): `https://academy.yourdomain.com/api/v1/...`
- **Swagger** (proxied): `https://academy.yourdomain.com/api/docs`

> **Important**: The backend listens on **port 5000** (per `backend/.env`). The frontend runs on **port 3000** in production.

---

## Prerequisites (Droplet/VM)

- Ubuntu 22.04+ (recommended)
- DNS A record: `academy.yourdomain.com` → your server IP
- Node.js 20.x (matches CI in `.github/workflows/deploy.yml`)
- PostgreSQL 16 (or Docker Compose if you prefer)
- Nginx
- PM2
...
Use your preferred Node 20 install method. Example with NodeSource:
...
sudo chown -R $USER:$USER /var/www/academy
cd /var/www/academy
git clone <your-repo-url> .
```

Create environment files (see `.env.example` in repo root):
...
### Prisma version pin (required)

This project uses `datasource.url` in `schema.prisma`, which requires **Prisma v6** (Prisma 7 removed this schema option).
...
Create `/var/www/academy/ecosystem.config.js`:
...
server {
  ...
  # Certbot will update this server block for TLS automatically.
...
server {
  ...
  ssl_certificate     /etc/letsencrypt/live/academy.yourdomain.com/fullchain.pem;
  ssl_certificate_key /etc/letsencrypt/live/academy.yourdomain.com/privkey.pem;

  # Request size limit (for PDFs, etc.)
  client_max_body_size 25M;

  # Security headers (Helmet also adds app-level protection)
...
After Nginx is enabled and DNS is pointing correctly:

```bash
sudo certbot --nginx -d academy.yourdomain.com
```

Auto-renew is installed by default on most distros. Verify:
...
- `https://academy.yourdomain.com/` loads
- `https://academy.yourdomain.com/api/docs` loads (Swagger)
- `https://academy.yourdomain.com/api/v1/books` responds
- Login works (seed admin for smoke test): `admin@multihat.dev` / `AdminSecure!2026` (**change in production**)
...
- [ ] Swagger docs accessible at `https://api.multihat.dev/api`
